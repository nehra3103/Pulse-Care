...
export default function App() {
  ...
  const getActiveUserForHeader = () => {
    if (currentRole === 'PATIENT') return activePatient;
    if (currentRole === 'DOCTOR') return activeDoctor ? { name: activeDoctor.name, avatar: activeDoctor.avatar } : null;
    return { name: 'Admin', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=RajeshAdmin' };
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans selection:bg-blue-600 selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {loading && doctors.length === 0 ? (
          <div className="py-24 text-center space-y-3">
            ...
            <p className="text-xs font-medium text-slate-500">Loading Healthcare Platform...</p>
          </div>
        ) : (
          <>
      ...
      <footer className="border-t border-slate-200 bg-white py-5 text-center text-xs text-slate-500 mt-auto">
        <p>© 2026 PulseCare Healthcare Platform • Minimal & Human-Centric Medical Follow-Up System</p>
      </footer>

      {/* Email Inbox / System Notification Drawer */}
      <NotificationDrawer
